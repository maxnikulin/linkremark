...
Add-on preferences has the "Request permissions on demand" options
that controls whether necessary permissions are requested
when an extension action is invoked. It is enabled by default.
