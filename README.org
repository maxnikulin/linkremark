...
It is necessary to capture frames loaded from other site
or a group of selected (highlighted) tabs in Firefox.
An alternative is to request access to particulars site
on demand and maybe I will implement such option in future.
Per-action permission requests may be annoying
and sometimes will require second capture attempt
due to restriction of context when permission request
may be shown.

By default extension has access only to the current tab
and in some cases could not inspect subframes.
...
Necessary to reliably restore tree of nested frames.
...
so it can be just pasted to a text editor.
...
or [[info:org#Capture][info "(org) Capture"]] is the next step.
...
#+end_example
You can add such entry using =M-x customize-variable RET org-capture-templates=.
...
#+end_example
Certainly Emacs server should be running: =M-x server-start=
and org-protocol should be loaded: =M-: (require 'org-protocol)=.
...
[fn:addons] Firefox: "Add-ons and Themes" from hamburger menu
...
[fn:snap] There is a problem with browsers distriuted
...
Open LinkRemark settings page (from context menu for the browser action button
or from preview and debug info page). Set "Name of native messaging backend
