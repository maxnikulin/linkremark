...
Chrome: "Modify data you copy and paste".

Hopefully in Firefox the extension works a bit better with this permission.
It is not strictly necessary however. The extension is still able
to copy text to clipboard in response to user action even without
this permission. The only drawback is transient preview tab in rare cases
(e.g. for PDF files). Native messaging backend does not use clipboard at all
and org-protocol touches clipboard only in non-default configuration.

Full functionality is available even with disabled permission.
