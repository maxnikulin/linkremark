...
Personally I am a bit afraid that some web page may try to inject
some malicious content into my notes, so I consider exporting
through a native messaging application as a safer option.
This repository contains simple variant in
...
#+end_example
and should specify "native-messaging" and =lr_emacsclient=
