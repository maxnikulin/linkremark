...
async function lrCheckFrameScriptsForbidden(tab, wrappedFrame) {
	...
	try {
		...
		console.warn("lrCheckFrameScriptsForbidden: tab %o frame %o error %o",
			tab, frameId, ex);
	}
	return null;
}
...
async function lrExecuteFrameScript(tab, wrappedFrame, file, property) {
	...
	try {
		...
		try {
			const checked = wrappedFrame.scripts_forbidden != null;
			if (!checked) {
				wrappedFrame.scripts_forbidden = await lrCheckFrameScriptsForbidden(tab, wrappedFrame);
			}
			if (wrappedFrame.scripts_forbidden) {
				// Do not pollute extension console with unavoidable errors.
				error = null;
			}
		} catch (check_ex) {
			console.error("lrExecuteFrameScript: ignoring error during permission check %o", check_ex);
		}
	}
	if (error) {
		console.error(
