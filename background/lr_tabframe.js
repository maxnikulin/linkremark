...
function focusedFrameChain(frameMap) {
	...
	for (const frameInfo of frameMap.values()) {
		...
		const referrer = frameInfo.referrer && frameInfo.referrer.result;
...
async function lrFrameChainByClickData(tab, frameMap, clickData) {
	...
	try {
		await Promise.all(chain.map(
			wrappedFrame =>
				lrExecuteFrameScript(tab, wrappedFrame, "content_scripts/referrer.js", "referrer")));
	} catch (ex) {
		console.error("lrFrameChainByClickData: capture: continue despite the error %s %o", ex, ex);
	}
	return chain;
}
...
async function lrFrameChainGuessSelected(tab, frameMap) {
	try {
		await Promise.all([...frameMap.values()].map(
			wrappedFrame =>
				lrExecuteFrameScript(tab, wrappedFrame, "content_scripts/referrer.js", "referrer")));
	} catch (ex) {
		console.error("lrFrameChainGuessSelected: capture: continue despite the error %s %o", ex, ex);
	}
...
async function lrGatherTabInfo(tab, clickData, activeTab) {
	...
	try {
		...
		console.error(
			"lrGatherTabInfo: meta or referrer: continue despite the error %s %o", ex, ex);
	}
	try {
		if (tab && tab.id >= 0) {
			let script, target;
